...
def main(city):
    ...
    def _plot(city_df, path):
        ...
        for index_stm, item in tqdm(city_df.groupby(['Index_stm'])):
            ...
            ref = item['MaxRef'].max()
            data.append([last_time, ref])
        data = pd.DataFrame(data=data, columns=['last_time', 'ref'])
        data['last_interval'] = data['last_time'].apply(process_box)
        if data['last_interval'].max() < 3:
            data = data.append({'last_interval': 3, 'ref': 0, 'index_stm': -1}, ignore_index=True)
        plt.figure(figsize=(12, 10))
        ax = sns.boxplot(data=data, x = "last_interval", y = "ref", showfliers=False)
        ax.set_xlabel('Duration(h)', fontsize=16) #设置x轴名称
        ax.set_ylabel('Reflectivity(dBZ)', fontsize=16) #设置y轴名称
        plt.savefig(path, dpi=200)
    
    base_path = './images/14_风暴最大反射因子'
