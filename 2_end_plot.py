...
def main(city):
	...
	def scatter_plot(end, path):
		...
		m.drawmeridians(np.arange(104, 108.5, 0.5), labels=[0, 0, 0, 1], color='grey',fontsize=18, ax=ax) # 设置经线
		for _, row in tqdm(end.iterrows()):
			x = row['Longitude']
			y = row['Latitude']
			x, y = m(x, y) # 坐标进行转换
	...
	base_path = './images/2_终止点'
	if not os.path.exists(base_path):
		os.mkdir(base_path)

	shadow_plot(end, f'{base_path}/{city}_all.png')
	for month in range(6, 9):
	    _end = end[end['Month'] == month]
	    shadow_plot(_end, f'{base_path}/{city}_{month}.png')
